...
//服务呼叫弹出框
element.addCall($("body"));
//会议修改弹出框
element.addMeeting($("body"));
//会议绑定弹出框
...
function align(){
	...
	if(screenWidth != screenHeight){
		document.documentElement.style.setProperty('--zoom', screenHeight/screenWidth);
		document.documentElement.style.setProperty('--h', screenHeight/480);
		if(!utils.isnull(sum)){
		...
		if(screenWidth > screenHeight){
			$("body").css("width",screenHeight)
			$("body").css("height",screenHeight)
		}else{
			$("body").css("width",screenWidth)
			$("body").css("height",screenWidth)
		}
	}
...
$(function(){
	...
	$(".back").on("click",function(){
		...
		$(".middle").css("display","none")
		$($(".middle")[0]).css("display","inline-block")
	})
	...
	//更多按钮
	$("html").find(".module-box-title").on("click","span",function (event) {
			...
			if(status != null){
				...
				if(event.originalEvent.isPattern != null){
					...
					if ($(this).hasClass("self-lock-active")) {
						...
						if(span != null && span.includes("%") && event.originalEvent.isTrusted){
							$(this).find(".module-box-adjust-change span").html("0%")
							brigh = 0
						}
					    json[control] = "false";
